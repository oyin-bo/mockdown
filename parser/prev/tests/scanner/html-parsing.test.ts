...
describe('HTML Parsing and Disambiguation', () => {
  describe('HTML Block Hinting', () => {
    test('hint persists until blank line', () => {
      ...
      // comment
      let k = scanner.scan();
      expect(k).toBe(SyntaxKind.HtmlComment);
      expect(scanner.getTokenFlags() & TokenFlags.ContainsHtmlBlock).toBeTruthy();
      // newline
      k = scanner.scan();
      ...
      // identifier 'para'
      k = scanner.scan();
      expect(k).toBe(SyntaxKind.Identifier);
      expect(scanner.getTokenFlags() & TokenFlags.ContainsHtmlBlock).toBeTruthy();
      // newline
      scanner.scan();
      // blank line newline
      ...
      k = scanner.scan();
      expect(k).toBe(SyntaxKind.Identifier);
      expect(scanner.getTokenFlags() & TokenFlags.ContainsHtmlBlock).toBeFalsy();
    });
    ...
    test('unterminated RAWTEXT sets Unterminated and error', () => {
      ...
      // opening tag
      expect(scanner.scan()).toBe(SyntaxKind.HtmlText);
      // content token
      const k = scanner.scan();
      expect(k).toBe(SyntaxKind.HtmlText);
      expect(scanner.getTokenFlags() & TokenFlags.IsInRawText).toBeTruthy();
      expect(scanner.getTokenFlags() & TokenFlags.Unterminated).toBeTruthy();
    ...
    test('unterminated RCDATA sets Unterminated and error', () => {
      ...
      let k = scanner.scan();
      expect(k).toBe(SyntaxKind.HtmlText);
      expect(scanner.getTokenFlags() & TokenFlags.IsInRcdata).toBeTruthy();
      // entity chunk
      k = scanner.scan();
      expect(k).toBe(SyntaxKind.HtmlText);
    ...
    test('CDATA/DOCTYPE/PI at line start enable hint and clear on blank', () => {
      ...
      let k = scanner.scan();
      expect(k).toBe(SyntaxKind.HtmlCDATA);
      expect(scanner.getTokenFlags() & TokenFlags.ContainsHtmlBlock).toBeTruthy();
      scanner.scan(); // nl
      k = scanner.scan(); // 'para'
      expect(k).toBe(SyntaxKind.Identifier);
      ...
      k = scanner.scan();
      expect(k).toBe(SyntaxKind.HtmlDoctype);
      expect(scanner.getTokenFlags() & TokenFlags.ContainsHtmlBlock).toBeTruthy();

      // Processing Instruction
      scanner.setText(`<?xml foo?>\npara`);
      k = scanner.scan();
      expect(k).toBe(SyntaxKind.HtmlProcessingInstruction);
      expect(scanner.getTokenFlags() & TokenFlags.ContainsHtmlBlock).toBeTruthy();
    });
  });
  ...
  describe('Autolink flags', () => {
    test('URL autolink flag set', () => {
      ...
      expect(token).toBe(SyntaxKind.HtmlText);
      expect(scanner.getTokenFlags() & TokenFlags.IsAutolinkUrl).toBeTruthy();
    });
    ...
    test('Email autolink flag set', () => {
      ...
      const token = scanner.scan();
      expect(token).toBe(SyntaxKind.HtmlText);
      expect(scanner.getTokenFlags() & TokenFlags.IsAutolinkEmail).toBeTruthy();
    });
  });
  ...
  describe('Raw-text and RCDATA elements', () => {
    test('script raw-text content and closing tag', () => {
      ...
      expect(scanner.scan()).toBe(SyntaxKind.HtmlText); // <script>
      const contentKind = scanner.scan();
      expect(contentKind).toBe(SyntaxKind.HtmlText);
      expect(scanner.getTokenFlags() & TokenFlags.IsInRawText).toBeTruthy();
      // Next should be closing tag start
      const ltSlash = scanner.scan();
      expect(ltSlash).toBe(SyntaxKind.LessThanSlashToken);
    });

    test('style raw-text content', () => {
      const scanner = createScanner();
      scanner.setText('<style>.x{display:none}</style>');
      expect(scanner.scan()).toBe(SyntaxKind.HtmlText); // <style>
      const contentKind = scanner.scan();
      expect(contentKind).toBe(SyntaxKind.HtmlText);
      expect(scanner.getTokenFlags() & TokenFlags.IsInRawText).toBeTruthy();
    });
    ...
    test('textarea RCDATA content with entity stays active', () => {
      ...
      expect(scanner.scan()).toBe(SyntaxKind.HtmlText); // <textarea>
      ...
      // Closing tag
      k = scanner.scan();
      expect(k).toBe(SyntaxKind.LessThanSlashToken);
    });

    test('title RCDATA content and closing', () => {
      const scanner = createScanner();
      scanner.setText('<title>Fish &amp; Chips</title>');
      expect(scanner.scan()).toBe(SyntaxKind.HtmlText); // <title>
      let k = scanner.scan();
      expect(k).toBe(SyntaxKind.HtmlText);
      expect(scanner.getTokenFlags() & TokenFlags.IsInRcdata).toBeTruthy();
      k = scanner.scan();
  ...
  describe('HTML Comments', () => {
    ...
    test('handles unterminated comment', () => {
      ...
      const token = scanner.scan();
      expect(token).toBe(SyntaxKind.HtmlComment);
      expect(scanner.getTokenFlags() & TokenFlags.Unterminated).toBeTruthy();
      expect(scanner.getTokenFlags() & TokenFlags.ContainsHtml).toBeTruthy();
    });
  ...
  describe('CDATA Sections', () => {
    ...
    test('handles unterminated CDATA', () => {
      ...
      const token = scanner.scan();
      expect(token).toBe(SyntaxKind.HtmlCDATA);
      expect(scanner.getTokenFlags() & TokenFlags.Unterminated).toBeTruthy();
    });
  });
  ...
  describe('Processing Instructions', () => {
    ...
    test('handles unterminated processing instruction', () => {
      ...
      const token = scanner.scan();
      expect(token).toBe(SyntaxKind.HtmlProcessingInstruction);
      expect(scanner.getTokenFlags() & TokenFlags.Unterminated).toBeTruthy();
    });
  });
  ...
  describe('HTML vs Text Disambiguation', () => {
    ...
    test('blockquote vs HTML tag disambiguation', () => {
      ...
      const token = scanner.scan();
      expect(token).toBe(SyntaxKind.BlockquoteToken);
      expect(scanner.getTokenFlags() & TokenFlags.IsAtLineStart).toBeTruthy();
    });
