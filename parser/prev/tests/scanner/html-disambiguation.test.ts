...
describe('HTML Tags and Disambiguation', () => {
  ...
  expect(s.scan()).toBe(SyntaxKind.HtmlText);
  expect(s.getTokenFlags() & TokenFlags.ContainsHtml).toBeTruthy();
  });
  ...
  expect(s2.scan()).toBe(SyntaxKind.HtmlComment);
  expect(s2.getTokenValue()).toBe(' This is a comment ');
  expect(s2.getTokenFlags() & TokenFlags.ContainsHtml).toBeTruthy();
  });

  test('scans unterminated HTML comment', () => {
  const s3 = createScanner();
  s3.setText('<!-- Unterminated comment');
  expect(s3.scan()).toBe(SyntaxKind.HtmlComment);
  expect(s3.getTokenFlags() & TokenFlags.Unterminated).toBeTruthy();
  expect(s3.getTokenFlags() & TokenFlags.ContainsHtml).toBeTruthy();
  });
  ...
  expect(s4.scan()).toBe(SyntaxKind.HtmlCDATA);
  expect(s4.getTokenValue()).toBe('Some data');
  expect(s4.getTokenFlags() & TokenFlags.ContainsHtml).toBeTruthy();
  });
  ...
  expect(s5.scan()).toBe(SyntaxKind.BlockquoteToken);
  expect(s5.getTokenFlags() & TokenFlags.IsAtLineStart).toBeTruthy();
  });
});
