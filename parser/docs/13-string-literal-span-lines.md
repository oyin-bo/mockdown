...
### Phase 0.0: Extended Normalisation Test Inputs (pre-start)

Before touching scanner internals, extend the existing scanner test suites with a broad set of normalization-focused inputs. These inputs exercise the current single-line normalization logic only (no cross-line spanning). Add each input below (exact raw markdown text) to the matching test file / suite so that they live near related tests.

Group by existing test suites and list the raw Markdown inputs (no assertions here). Developers can paste these into the appropriate test files as new test cases or data-driven inputs.

Tests for `parser/tests/1-basic.test.ts` (general normalization + whitespace). Each line in the snippet below is a separate test input, where annotated assertions need to be added accordingly:
```
Hello  world
...
Backtick `code` in-line
Ampersand &amp; &lt; &gt;
Unicode combining: e01 and emoji 👍🏼
```

Tests for `parser/tests/1-edge-cases.test.ts` (edge whitespace, hard breaks, blank runs). Each line in the snippet below is a separate test input, where annotated assertions need to be added accordingly:
```
...
Tests for `parser/tests/3-inline-formatting.test.ts` (inline punctuation/emphasis/emoji/entities). Each line in the snippet below is a separate test input, where annotated assertions need to be added accordingly:
```
...
Tests for `parser/tests/2-examples.test.ts` (examples and larger mixed lines). Each line in the snippet below is a separate test input, where annotated assertions need to be added accordingly:
```
...
Notes:
- These inputs must be used only to validate existing single-line normalization behavior. Do not enable or assert cross-line joining for these cases.
- For all these tests add also a <br> at the very end, and assert its position in the annotated Markdown. This is to verify that complex string patterns don't over- or under-consume characters inside scanner and don't disrupt the scanning of the following tokens.
