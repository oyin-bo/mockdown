...
@1 HtmlProcessingInstruction "<?target?>"`;
  ...
  test('processing instruction with quoted attributes', () => {
    const tokenTest = `
<?xml version="1.0" encoding="UTF-8"?>
1
@1 HtmlProcessingInstruction "<?xml version=\\"1.0\\" encoding=\\"UTF-8\\"?>"`;
    expect(verifyTokens(tokenTest)).toBe(tokenTest);
  });

  test('processing instruction with single-quoted attribute', () => {
    const tokenTest = `
<?target attr='value'?>
1
@1 HtmlProcessingInstruction "<?target attr='value'?>"`;
    expect(verifyTokens(tokenTest)).toBe(tokenTest);
  });
