...
describe('Context and Advanced Features', () => {
    describe('Context Sensitivity', () => {
        test('tracks line start context', () => {
            ...
            const hashToken = tokens.find(t => t.kind === 11 /* SyntaxKind.HashToken */);
            expect(hashToken && (hashToken.flags & 32 /* TokenFlags.IsAtLineStart */)).toBeTruthy();
        });
        test('tracks preceding line break', () => {
            const tokens = scanTokens('line1\nline2');
            const identifierTokens = tokens.filter(t => t.kind === 36 /* SyntaxKind.Identifier */);
            expect(identifierTokens[1] && (identifierTokens[1].flags & 2 /* TokenFlags.PrecedingLineBreak */)).toBeTruthy();
        });
    });
    describe('Lookahead and Rescanning', () => {
        test('supports lookahead without affecting state', () => {
            ...
            expect(lookaheadResult).toBe(2 /* SyntaxKind.LessThanToken */);
