...
describe('ATX Headings and Lists', () => {
  ...
  describe('ATX Headings', () => {
    test('scans ATX heading at line start', () => {
      ...
      expect(s.getTokenFlags() & TokenFlags.IsAtLineStart).toBeTruthy();
    });
  ...
  describe('Lists', () => {
    test('scans unordered list asterisk', () => {
      ...
      s2.setText('* Item 1');
      expect(s2.scan()).toBe(SyntaxKind.AsteriskToken);
      expect(s2.getTokenFlags() & TokenFlags.IsAtLineStart).toBeTruthy();
    });
    ...
    test('scans unordered list dash', () => {
      ...
      s3.setText('- Item 1');
      expect(s3.scan()).toBe(SyntaxKind.DashToken);
      expect(s3.getTokenFlags() & TokenFlags.IsAtLineStart).toBeTruthy();
    });
    ...
    test('scans unordered list plus', () => {
      ...
      s4.setText('+ Item 1');
      expect(s4.scan()).toBe(SyntaxKind.PlusToken);
      expect(s4.getTokenFlags() & TokenFlags.IsAtLineStart).toBeTruthy();
    });
    ...
    test('ordered list marker with parenthesis delimiter', () => {
      ...
      expect(k).toBe(SyntaxKind.NumericLiteral);
      const flags = scanner.getTokenFlags();
      expect(flags & TokenFlags.IsOrderedListMarker).toBeTruthy();
      expect(flags & TokenFlags.OrderedListDelimiterParen).toBeTruthy();
    ...
    test('tab indent prevents ordered list marker (column >=4)', () => {
      ...
      expect(k2).toBe(SyntaxKind.NumericLiteral);
      expect(scanner.getTokenFlags() & TokenFlags.IsOrderedListMarker).toBeFalsy();
    });
  });
});
